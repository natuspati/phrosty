...
class TestProfileView:
    async def test_authenticated_user_can_view_other_users_profile(
            self, app: FastAPI, authorized_client: TestClient, test_user: UserInDB, test_user2: UserInDB
    ) -> None:
    ...
    async def test_unregistered_users_cannot_access_other_users_profile(
            self, app: FastAPI, client: TestClient, test_user2: UserInDB
    ) -> None:
    ...
    async def test_no_profile_is_returned_when_username_matches_no_user(
            self, app: FastAPI, authorized_client: TestClient
    ) -> None:
        res = await authorized_client.get(
...
class TestProfileManagement:
    ...
    async def test_user_can_update_own_profile(
            self, app: FastAPI, authorized_client: TestClient, test_user: UserInDB, attr: str, value: str,
    ) -> None:
